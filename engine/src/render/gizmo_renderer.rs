...
impl GizmoRenderer {
    pub fn new(cc: &eframe::CreationContext, camera_uniform_buffer: &wgpu::Buffer, samples: u32) -> Self {
        let render_state = cc.wgpu_render_state.as_ref().unwrap();
        let device = &render_state.device;
        let queue = &render_state.queue;

        let circle_instance_buffer = ResizableBuffer::new(BufferUsages::VERTEX | BufferUsages::COPY_DST);
        let cube_instance_buffer = ResizableBuffer::new(BufferUsages::VERTEX | BufferUsages::COPY_DST);
        let instance = GizmoInstance {
            color: Vec4::default().into(),
            enable_normals: 0,
            use_uv_colors: 1,
        };
        let lines_instance_buffer = device.create_buffer_init(&wgpu_buffer_init_desc(BufferUsages::VERTEX, &[instance]));
        let points_instance_buffer = device.create_buffer_init(&wgpu_buffer_init_desc(BufferUsages::VERTEX, &[instance]));

        let gizmo_shader = device.create_shader_module(
            wgpu::include_wgsl!("../../../assets/shaders/gizmos.wgsl")
        );
        ...
        let gizmo_pipeline_line_list = Self::create_pipeline(
            render_state, "gizmo_pipeline_line_list",
        ...
        let gizmo_pipeline_line_strip = Self::create_pipeline(
            render_state, "gizmo_pipeline_line_strip",
            &gizmo_pipeline_layout,
            &gizmo_shader,
        ...
        let gizmo_pipeline_point_list = Self::create_pipeline(
            render_state, "gizmo_pipeline_point_list",
            &gizmo_pipeline_layout,
            &gizmo_shader,
    ...
    fn load_buffers(&mut self, device: &wgpu::Device, queue: &wgpu::Queue) {
        self.circle_instance_buffer.write_buffer(device, queue, self.circle_list.as_slice());
        self.cube_instance_buffer.write_buffer(device, queue, self.cube_list.as_slice());
    ...
    pub fn render_gizmos<'a>(
        ...
        if self.wire_circle_mesh.instances.len() > 0 {
            ...
            render_pass.set_vertex_buffer(2, self.circle_instance_buffer.get_wgpu_buffer().slice(..));
            render_pass.draw(
        ...
        if self.wire_cube_mesh.instances.len() > 0 {
            ...
            render_pass.draw_indexed(
                0..(self.wire_cube_mesh.indices.len() as u32), 0,
